...
const BookSlot = () => {
    ...
    const { getUnparkedCars } = useContext(carContext);

    useEffect(() => {
        const fetchCars = async () => {
            try {
                const res = await getUnparkedCars();
                setCarSaved(res.data);
            } catch (error) {
    ...
    const handleSubmit = async() => {
        ...
        if(fromTime>=toTime){
            toast.error("Can't able to Park");
            return;
        }
    ...
    return (
        <div className="flex w-full h-[120vh] left-0 top-0 overflow-x-hidden">
            {/* 1st Column */}
