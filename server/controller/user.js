...
exports.set_parking_slot = async (req, res) => {
    const {isEmpty,parking_id} = req.body;
    if (!parking_id) {
    ...
    try {
        db.query(
            ...
            (error, results) => {
                ...
                return res.status(200).json({ message: 'Parking slots Updated successfully', data: results, success: true });
            }
        );
    } catch (err) {
        console.error("Unexpected error:", err);
